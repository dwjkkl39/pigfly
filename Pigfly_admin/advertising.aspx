<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="advertising.aspx.cs" Inherits="Pigfly_admin.advertising" %>
   ...
   <div class="border clearfix">
	...
	<tbody>
      ...
      <tr>
       <td></td>
       <td><%#Eval("aid") %></td>
       <td><asp:Image ID="Image1" runat="server"  ImageUrl='<%#Eval("aimg") %>'/></td>
	   <td><%#Eval("mid") %></td>
       <td><%#Eval("aname") %></td>
       <td><%#Eval("ashow") %></td>
      <td id="tbNumber" runat="server"><span class="label label-success radius"><%# Convert.ToInt32( Eval("sbefore"))==1?"展示":"<p style='color:red'>不展示</p>" %></span></td>
      <td class="td-manage">
       <asp:Button ID="Button2"  class="btn btn-danger radius" type="button" runat="server" Text="启用"  CommandArgument='<%#Eval("aid") %>' CommandName="update1"/>
       <asp:Button ID="UPdateEdit"  class="btn btn-danger radius" type="button" runat="server" Text="未启用"  CommandArgument='<%#Eval("aid") %>' CommandName="update"/>
        <asp:Button ID="Button1"  class="btn btn-danger radius" type="button" runat="server" Text="删除"  CommandArgument='<%#Eval("aid") %>' CommandName="delete"/>
       </td>
      </tr>
			</ItemTemplate>
			</asp:Repeater>
